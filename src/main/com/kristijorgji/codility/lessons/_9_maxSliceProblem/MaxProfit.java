...
/**
 * @see <a href="https://app.codility.com/programmers/lessons/9-maximum_slice_problem/max_profit/">MaxProfit</a>
 *
...
public class MaxProfit {
    /**
     * 100% solution based on Kadane's algorithm
     *
     * The idea is to keep track only of the maximum profit so far, considering we do not need to return the indexes.
     * We want to do that for values A[P] and A[Q] that meet the condition 0 <= Q <=P < N
     * This condition is always met because we iterate only with one loop from [0, N-1] and Q <= P,
     */
    public int optimizedSolution(int[] A)
    {
        ...
        for (int price : A) {
            min = Math.min(min, price); //the order of this and the instruction below can also be interchanged
